...
int postgres_connect(uv_loop_t *loop, postgres_t *postgres) {
    ...
    if ((error = uv_poll_start(&postgres->poll, UV_WRITABLE, postgres_on_connect))) { ERROR("uv_poll_start\n"); return error; } // int uv_poll_start(uv_poll_t* handle, int events, uv_poll_cb cb)
    return error;
}
...
int postgres_reset(postgres_t *postgres) {
    ERROR("postgres=%p, postgres->request=%p\n", postgres, postgres->request);
    int error = 0;
...
//    if (uv_is_active((uv_handle_t *)&postgres->poll)) if ((error = uv_poll_stop(&postgres->poll))) { ERROR("uv_poll_stop\n"); return error; } // int uv_is_active(const uv_handle_t* handle); int uv_poll_stop(uv_poll_t* poll)
    ...
    if ((error = uv_poll_start(&postgres->poll, UV_WRITABLE, postgres_on_reset))) { ERROR("uv_poll_start\n"); return error; } // int uv_poll_start(uv_poll_t* handle, int events, uv_poll_cb cb)
...
void postgres_listen(postgres_t *postgres) {
    ...
    if (uv_poll_start(&postgres->poll, UV_WRITABLE, postgres_on_query)) { ERROR("uv_poll_start\n"); return; } // int uv_poll_start(uv_poll_t* handle, int events, uv_poll_cb cb)
}
...
void postgres_on_connect(uv_poll_t *handle, int status, int events) { // void (*uv_poll_cb)(uv_poll_t* handle, int status, int events)
    ...
    switch (PQconnectPoll(postgres->conn)) { // PostgresPollingStatusType PQconnectPoll(PGconn *conn)
        case PGRES_POLLING_FAILED: ERROR("PGRES_POLLING_FAILED\n"); postgres_reset(postgres); break;
        case PGRES_POLLING_READING: /*DEBUG("PGRES_POLLING_READING\n"); */if (uv_poll_start(&postgres->poll, UV_READABLE, postgres_on_connect)) ERROR("uv_poll_start\n"); break; // int uv_poll_start(uv_poll_t* handle, int events, uv_poll_cb cb)
        case PGRES_POLLING_WRITING: /*DEBUG("PGRES_POLLING_WRITING\n"); */if (uv_poll_start(&postgres->poll, UV_WRITABLE, postgres_on_connect)) ERROR("uv_poll_start\n"); break; // int uv_poll_start(uv_poll_t* handle, int events, uv_poll_cb cb)
        case PGRES_POLLING_OK: /*DEBUG("PGRES_POLLING_OK\n");*/ postgres_listen(postgres); break;
        default: break;
    }
}
...
void postgres_on_reset(uv_poll_t *handle, int status, int events) { // void (*uv_poll_cb)(uv_poll_t* handle, int status, int events)
    ...
    postgres_t *postgres = (postgres_t *)handle->data;
    if (status) { ERROR("status=%i\n", status); postgres_reset(postgres); return; }
    switch (PQresetPoll(postgres->conn)) { // PostgresPollingStatusType PQresetPoll(PGconn *conn)
        case PGRES_POLLING_FAILED: ERROR("PGRES_POLLING_FAILED\n"); postgres_reset(postgres); break;
        case PGRES_POLLING_READING: /*DEBUG("PGRES_POLLING_READING\n"); */if (uv_poll_start(&postgres->poll, UV_READABLE, postgres_on_reset)) ERROR("uv_poll_start\n"); break; // int uv_poll_start(uv_poll_t* handle, int events, uv_poll_cb cb)
        case PGRES_POLLING_WRITING: /*DEBUG("PGRES_POLLING_WRITING\n"); */if (uv_poll_start(&postgres->poll, UV_WRITABLE, postgres_on_reset)) ERROR("uv_poll_start\n"); break; // int uv_poll_start(uv_poll_t* handle, int events, uv_poll_cb cb)
        case PGRES_POLLING_OK: /*DEBUG("PGRES_POLLING_OK\n");*/ postgres_listen(postgres); break;
        default: break;
    }
}
...
void postgres_on_query(uv_poll_t *handle, int status, int events) { // void (*uv_poll_cb)(uv_poll_t* handle, int status, int events)
    ...
    postgres_t *postgres = (postgres_t *)handle->data;
    if (status) { ERROR("status=%i\n", status); postgres_reset(postgres); return; }
    if (PQstatus(postgres->conn) != CONNECTION_OK) { ERROR("PQstatus!=CONNECTION_OK\n"); postgres_reset(postgres); return; } // ConnStatusType PQstatus(const PGconn *conn)
    ...
    if (events & UV_WRITABLE) switch (PQflush(postgres->conn)) { // int PQflush(PGconn *conn);
        case 0: /*DEBUG("No data left to send\n"); */if (uv_poll_start(&postgres->poll, UV_READABLE, postgres_on_query)) ERROR("uv_poll_start\n"); break; // int uv_poll_start(uv_poll_t* handle, int events, uv_poll_cb cb)
...
void postgres_response(PGresult *result, postgres_t *postgres) {
    ...
    if (response_write(client, PQgetvalue(result, 0, 0), PQgetlength(result, 0, 0))) { ERROR("response_write\n"); client_close(client); } // char *PQgetvalue(const PGresult *res, int row_number, int column_number); int PQgetlength(const PGresult *res, int row_number, int column_number)
}
