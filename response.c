...
int response_write(context_t *context) {
    ...
    if (uv_write(&wr->req, (uv_stream_t *)&context->client, &wr->buf, 1, response_on_write)) { // int uv_write(uv_write_t* req, uv_stream_t* handle, const uv_buf_t bufs[], unsigned int nbufs, uv_write_cb cb)
        ERROR("uv_write\n");
        free(wr);
        return errno;
    }
