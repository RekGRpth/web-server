...
//    if ((error = client->tcp.flags > MAX_FLAG)) { ERROR("client=%p, client->tcp.flags=%u\n", client, client->tcp.flags); return error; }
    ...
    if ((error = xbuf_xcat(&response->xbuf, STATUS, client->parser.http_major, client->parser.http_minor, http_status_str(code)) <= 0)) { ERROR("xbuf_xcat\n"); response_free(response); return error; }
    if ((error = xbuf_xcat(&response->xbuf, HEADERS, length) <= 0)) { ERROR("xbuf_xcat\n"); response_free(response); return error; }
...
static response_t *response_init() {
    ...
    if (!xbuf_init(&response->xbuf)) { ERROR("xbuf_init\n"); response_free(response); return NULL; }
